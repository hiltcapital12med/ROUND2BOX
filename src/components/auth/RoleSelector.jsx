...
export default function RoleSelector({ onRoleSelect, loading }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 relative overflow-hidden">
      ...
      <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-brand-accent opacity-5 rounded-full blur-3xl"></div>
      ...
      <div className="relative z-10 w-full max-w-lg bg-white border border-gray-200 p-8 rounded-3xl shadow-lg">
          ...
          <button
            ...
            className={`relative group p-6 rounded-2xl border-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 ${
              selectedRole === 'user' 
                ...
                : 'border-white/20 bg-white/5 hover:border-brand-secondary/50 hover:bg-white/10'
            } disabled:opacity-50 disabled:cursor-not-allowed`}
          >
            {/* Icono */}
            <div className={`text-4xl transition-colors ${selectedRole === 'user' ? 'text-brand-accent' : 'text-brand-secondary'}`}>
              <Barbell size={48} weight="duotone" />
            </div>
            
            {/* Texto */}
            <div className="text-center">
              <h3 className="text-gray-800 font-bold text-lg">ATLETA</h3>
              <p className="text-gray-800/60 text-xs mt-1">Entrenar y mejorar</p>
            </div>

            {/* Indicador de selección */}
            {selectedRole === 'user' && (
              <div className="absolute top-3 right-3 w-3 h-3 bg-brand-accent rounded-full animate-pulse"></div>
            )}
          </button>
          ...
          <button
            ...
            className={`relative group p-6 rounded-2xl border-2 transition-all duration-300 flex flex-col items-center justify-center gap-4 ${
              selectedRole === 'trainer' 
                ? 'border-brand-secondary bg-brand-secondary/10 shadow-lg shadow-brand-secondary/20' 
                : 'border-white/20 bg-white/5 hover:border-brand-secondary/50 hover:bg-white/10'
            } disabled:opacity-50 disabled:cursor-not-allowed`}
          >
            {/* Icono */}
            <div className={`text-4xl transition-colors ${selectedRole === 'trainer' ? 'text-brand-secondary' : 'text-brand-secondary'}`}>
              <Lightbulb size={48} weight="duotone" />
            </div>
            
            {/* Texto */}
            <div className="text-center">
              <h3 className="text-gray-800 font-bold text-lg">COACH</h3>
              <p className="text-gray-800/60 text-xs mt-1">Entrenar y enseñar</p>
            </div>

            {/* Indicador de selección */}
            {selectedRole === 'trainer' && (
              <div className="absolute top-3 right-3 w-3 h-3 bg-brand-secondary rounded-full animate-pulse"></div>
            )}
          </button>
        </div>
        ...
        <button
          ...
          className="w-full group relative flex items-center justify-center gap-2 bg-brand-accent text-gray-800 px-6 py-4 rounded-xl font-bold transition-all duration-300 hover:bg-red-700 hover:shadow-gold-glow hover:-translate-y-1 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-brand-accent disabled:hover:translate-y-0 overflow-hidden"
        >
          {/* Destello dorado */}
          <div className="absolute inset-0 w-full h-full bg-gradient-to-r from-transparent via-brand-secondary/20 to-transparent -translate-x-[100%] group-hover:translate-x-[100%] transition-transform duration-700 ease-in-out disabled:hidden"></div>
        ...
        <p className="text-center text-brand-secondary/50 text-xs mt-8">
