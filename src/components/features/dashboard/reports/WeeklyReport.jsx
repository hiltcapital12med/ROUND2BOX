...
export default function WeeklyReport() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold text-gray-800">Reporte Semanal</h3>

      {loading ? (
        <div className="text-gray-800/60 text-center py-8">Cargando...</div>
      ) : weeklyData.length === 0 ? (
        <div className="text-gray-800/60 text-center py-8">Sin datos de esta semana</div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="bg-black/40 rounded-lg p-4 border border-white/10">
              <p className="text-gray-800/60 text-sm">Promedio/Día</p>
              <p className="text-2xl font-bold text-brand-neon">{avgReservations}</p>
            </div>
            <div className="bg-black/40 rounded-lg p-4 border border-white/10">
              <p className="text-gray-800/60 text-sm">Clases</p>
          ...
          <div className="bg-black/40 rounded-lg p-4 border border-white/10 space-y-2 max-h-[200px] overflow-y-auto">
            <p className="text-gray-800 font-bold mb-3">Detalles por día</p>
            {weeklyData.map((day, idx) => (
              <div key={idx} className="flex justify-between items-center text-sm">
                <span className="text-gray-800/80">{day.day} ({day.fullDate})</span>
                <div className="flex gap-4">
                  <span className="text-brand-neon font-semibold">{day.reservations} reservas</span>
                  <span className="text-gray-800/60">{day.classes} clases</span>
                </div>
              </div>
            ))}
          </div>
